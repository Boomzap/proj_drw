...
[FriendlyName("Riddle")]
public class HOLogicRiddle : HOLogic
{
    ...
    protected override void GroupSelectedObjectsToCurrentAndFuture(ref List<HOFindableObject> validObjectsList, ref List<HOFindableObject> selectedObjectsList)
    {
        ...
        {
            totalToFind = currentObjects.Count + futureObjects.Count;
            Debug.LogWarning($"Riddle wanted {totalToFind} items, but we only came up with {currentObjects.Count}");
        }
