...
namespace Boomzap.Utilities
{
    public static class BZImagePacker
    {
        /// <summary>
        ...
        public static void Pack(NativeArray<Color32>[] buffers, int width, int height, int padding, out NativeArray<Color32> outPackedBuffer, out int outPackedBufferWidth, out int outPackedBufferHeight, out RectInt[] outPackedRect, out Vector2Int[] outUVTransform)
        {
            ...
            try
            {
                var tightRects = BZFindTightRectJob.Execute(buffers, width, height);
        ...
        static ImagePackNode InternalPack(RectInt[] rects, int padding)
        {
            ...
            // this is the only change jd's made : start with the biggest texture first, instead of the smallest.. this alone will stop the horrible packing heuristics
            root.rect = new RectInt(0, 0, (int)NextPowerOfTwo((ulong)sortedRects[0].rect.width), (int)NextPowerOfTwo((ulong)sortedRects[0].rect.height));
